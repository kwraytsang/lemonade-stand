  ...
  describe('get', () => {
    it('sends a GET request with JSON headers and returns the parsed body', async () => {
      ...
      (globalThis.fetch as jest.Mock).mockResolvedValue(
        mockFetchResponse({ ok: true, status: 200, json: async () => body }),
      );
    ...
    it('returns undefined for a 204 No Content response without reading a body', async () => {
      ...
      (globalThis.fetch as jest.Mock).mockResolvedValue(
        mockFetchResponse({ ok: true, status: 204, json: jsonSpy }),
      );
    ...
    it('throws an ApiError carrying the server message when the response is not ok', async () => {
      (globalThis.fetch as jest.Mock).mockResolvedValue(
        mockFetchResponse({
          ok: false,
          status: 404,
          json: async () => ({ message: 'Beverage type not found' }),
        }),
      );

      await expect(
        apiClient.get('/customer/beverage-types/missing'),
    ...
    it('throws an ApiError when the request fails', async () => {
      (globalThis.fetch as jest.Mock).mockResolvedValue(
        mockFetchResponse({
          ok: false,
          status: 400,
          json: async () => ({ message: ['customerName should not be empty'] }),
        }),
      );

      await expect(
        apiClient.post('/customer/orders', {}),
      ).rejects.toMatchObject({
        statusCode: 400,
